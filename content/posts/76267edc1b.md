...
Fedora 的开发者照着 Windows 和 macOS，搞了个 Plymouth：它可以像这几个操作系统一样，显示开机画面。正好我之前换到了 systemd-boot，可以默认禁用菜单，开机过程更加简洁了；现在再关掉日志，看起来也更舒服些。
...
至于主题设定，请自行查看 [Wiki](https://wiki.archlinux.org/title/Plymouth#Changing_the_theme)。我觉得默认的 `bgrt` 的 OEM 标志就挺好。

另外一个小小的调整就是，我用 Windows 的时候，主板的徽标显示是和操作系统连在一起的。尤其是我的小米笔记本，开机显示徽标有个很漂亮的动画。
...
启用 SimpleDRM 即可。*据说*在 AMD 显卡的机器上显示更好，但在我这儿似乎没用——还是有一段黑屏。
